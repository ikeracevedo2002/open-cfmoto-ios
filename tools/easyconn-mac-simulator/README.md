...
The test currently validates Bonjour discovery, PXC framing, client info,
serial validation, heartbeat, and media negotiation. A successful run will
currently end the media test with `No H.264 frame returned`; that is expected
until the iOS VideoToolbox encoder implements media command `114`.
